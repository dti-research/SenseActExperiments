...
#        print('rewards: {} \n\n'.format(rewards[i]))
    ...
    # Compute mean reward and its standard error
    rewards_mean = []
    rewards_std_err = []
    ...
    for r in rewards:
        ...
        for v in r:
            ...
            conf_reward_std_err.append(np.std(v)/math.sqrt(len(v)))
#            print('v: {} \n\n'.format(v))
        rewards_mean.append(conf_reward_means)
        rewards_std_err.append(conf_reward_std_err)
        print("std:err: {} \n\n".format(conf_reward_std_err))
        print('mean: {} \n\n'.format(conf_reward_means))
    ...
    for i in range(len(rewards_mean)):
        plt.plot(timesteps[i], rewards_mean[i], linewidth=2.0, label='Configuration {}'.format(i+1),color=colors[i])
        plt.fill_between(timesteps[i],
                     (np.array(rewards_mean[i]) - np.array(rewards_std_err[i])),
                     (np.array(rewards_mean[i]) + np.array(rewards_std_err[i])),
                     color=colors[i],
                     linewidth=0.0,
                     alpha=0.3)
