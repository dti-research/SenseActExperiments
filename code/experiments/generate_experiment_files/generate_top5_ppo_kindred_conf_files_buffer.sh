...
do
    ...
    python3 generate_conf_file.py \
            --filename=../ur5/ppo_kindred_example_buffer.yaml \
            --hid_size=${hid_size[$i]} \
            --num_hid_layers=${num_hid_layers[$i]} \
            --batch_size=${batch_size[$i]} \
            --opt_step_size=${step_size[$i]} \
            --opt_batch_size=${opt_batch_size[$i]} \
            --gamma=${gamma[$i]} \
            --lamda=${lamda[$i]} \
            --output_filename=ppo_conf_buffer_$i.yaml \
            --output_dir=../ur5/ppo_buffer/\
            --log_dir=artifacts/logs/ppo_buffer/$i
done
